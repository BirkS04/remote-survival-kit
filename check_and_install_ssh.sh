...
# Wenn Variablen nicht vom aufrufenden Skript (install.sh) gesetzt wurden, frage nach:
if [ -z "$TARGET_IP" ]; then
...
if ! grep -q "^Host $TARGET_ALIAS$" "$CONFIG_FILE" 2>/dev/null; then
...
# Wir fragen nur ab, wenn SECURE_CHOICE nicht schon von außen übergeben wurde
